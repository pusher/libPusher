...
{
	if (self.delegate && [self.delegate respondsToSelector:@selector(channelDidConnect:)])
		[self.delegate channelDidConnect:self];
}

- (void)pusherDidDisconnect:(PTPusher *)pusher
{
	if (self.delegate && [self.delegate respondsToSelector:@selector(channelDidDisconnect:)])
